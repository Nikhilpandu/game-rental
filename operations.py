...
def Fetch_all_Products_created_by_seller(owner,product_json_file):
    ...
    '''Write your code below'''
    product_created_by_seller = []
    try:
        with open(product_json_file) as f:
            file_data = json.load(f)
        for data in file_data:
            if data["Seller Username"] == owner:
                product_created_by_seller.append(file_data["Product Title"])
    except JSONDecodeError:
            pass

    return product_created_by_seller
...
def Fetch_Product_By_ID(product_json_file,product_ID):
    ...
    try:
        ...
        for d in file_data:
            if d["Seller Username"] == product_ID:
                Product_By_ID.append(file_data["product ID"])
    except JSONDecodeError:
        pass
...
def Update_Product(Username,product_json_file,product_ID,detail_to_be_updated,new_value):
    ...
    with open(product_json_file,'r+') as f:
        ...
        for a in range(len(file_data)):
            if file_data[a]["Product ID"]==product_ID:
                file_data[a]["Seller Username"] == Username

                for details in detail_to_be_updated:
                    if details in file_data[a]["Product Title"]:
                        details = new_value
                    if details in file_data[a]["Product Type"]:
                        details = new_value
                    if details in file_data[a]["Price Per Day"]:
                        details = new_value
                    if details in file_data[a]["Total Stock Available"]:
                        details = new_value
...
def View_Cart(Username,gamers_json_file):
    ...
    with open(gamers_json_file) as f:
        ...
        for a in range(len(file_data)):
            if file_data[a]["Username"] == Username:
                return file_data[a]
    return file_data
...
def Update_User(gamers_json_file,Username,detail_to_be_updated,updated_detail):
    ...
    with open(gamers_json_file,'r+') as f:
        file_data= json.load(f)
        for a in range(len(file_data)):
            if file_data[a]["Seller Username"] == Username:

                for details in detail_to_be_updated:
                    if details in file_data[a]["email"]:
                        details = updated_detail
                    if details in file_data[a]["password"]:
                        details = updated_detail
                    if details in file_data[a]["contact number"]:
                        details = updated_detail
                f.seek(0)
                f.truncate()
                json.dump(file_data,f)
    ...
    try:
        ...
        for data in file_data:
            if data["Ordered By"] == Username:
                fetch_all_data.append9file_data["items"]
    except JSONDecodeError:
        pass
